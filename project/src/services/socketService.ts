...
class SocketService {
  ...
  private setupEventForwarding(): void {
    ...
    this.socket.on('user_initialized', (user: User) => {
      this.emit('user_initialized', user);
    });
    ...
    this.socket.on('user_typing', (data: { userId: string; username: string; recipientId?: string }) => {
      this.emit('user_typing', data);
    });

    this.socket.on('user_stopped_typing', (data: { userId: string; recipientId?: string }) => {
  ...
  startTyping(recipientId?: string): void {
    if (!this.socket) throw new Error('Not connected to server');
    this.socket.emit('start_typing', { recipientId });
  }
